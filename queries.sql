...
SELECT a.name AS animal_name, v.visit_date, vet.name AS vet_name, vet.age AS vet_age
FROM visits v
JOIN animals a ON v.animal_id = a.id
JOIN vets vet ON v.vet_id = vet.id
ORDER BY v.visit_date DESC
...
SELECT COUNT(*) AS count_of_mismatched_visits
FROM visits v
JOIN animals a ON v.animal_id = a.id
JOIN vets vt ON v.vet_id = vt.id
...
SELECT s.name AS recommended_specialty, COUNT(*) AS visits_count
FROM visits v
JOIN animals a ON v.animal_id = a.id
JOIN vets vt ON v.vet_id = vt.id
